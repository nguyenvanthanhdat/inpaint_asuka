...
# NOTE: check this function write
# from tqdm.contrib import tzip
# def encode_prompt(batch, text_encoders, tokenizers, proportion_empty_prompts, caption_column, is_train=True):
#     prompt_embeds_list = []
#     caption_column = caption_column.split(",")
#     # prompt_batch = batch[caption_column]
#     image = batch[caption_column[0]]
#     mask = batch[caption_column[1]]
#     align = tokenizers[0]
#     tokenizer_2 = tokenizers[1]
#     text_encoder_1 = text_encoders[0]
#     text_encoder_2 = text_encoders[1]
#     temp = []
#     for img, msk in tzip(image, mask):
#         img = mae.feature_extractor(images=img, do_rescale=True, return_tensors="pt").pixel_values
#         msk = mae.transform_mask(msk).unsqueeze(0)
#         mae_output = mae.forward(img, msk).logits.squeeze(0)
#         # align_output = align(mae_output)
#         # temp.append(align_output)
#         temp.append(mae_output)
#     # convert list to tensor
#     prompt_embeds = torch.stack(temp).squeeze(1)
#     # move prompt_embeds to align model device
#     prompt_embeds = prompt_embeds.to(next(align.parameters()).device)
#     print(f"prompt_embeds: {prompt_embeds.shape}")
#     context = align(prompt_embeds)
#     # print(image)
#     # print(mask)
#     captions = []
...
# TODO: calulate the mae output first
def compute_mae_encodings(batch, mae, mae_device):
    # images = batch.pop("image")
    # masks = batch.pop("mask")
    images = batch["image"]
    masks = batch["mask"]
    temp = []
    for img, msk in zip(images, masks):
        img = mae.feature_extractor(images=img, do_rescale=True, return_tensors="pt").pixel_values.to(mae_device)
        msk = mae.transform_mask(msk).unsqueeze(0).to(mae_device)
        mae_output = mae.forward(img, msk).logits.squeeze(0)
        temp.append(mae_output)
    # convert list to tensor
    mae_embeds = torch.stack(temp).squeeze(1)
    return {"mae_embeds": mae_embeds.cpu()}
...
def main(args):
    ...
    #     use_fast=False,
    # )
    # TODO: load unet in onnx format
    if args.unet_onnx:
        from optimum.onnxruntime import ORTStableDiffusionXLPipeline
        print(args.unet_onnx_device_ids)
        model_id = "stabilityai/stable-diffusion-xl-base-1.0"
        unet_list = []
        for device_id in args.unet_onnx_device_ids:
            base = ORTStableDiffusionXLPipeline.from_pretrained(
                model_id, 
                # provider="CUDAExecutionProvider",
                device=f"cuda:{device_id}",
            )
            unet_list.append(base.unet)
            del base
        print("Load UNet in ONNX format done")
    else:
    ...
    mae_device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    align_tokenizer_one.to(accelerator.device, dtype=weight_dtype)
    ...
    # Optimizer creation
    params_to_optimize = align_tokenizer_one.parameters()
    ...
    if args.caption_column is None:
        ...
        caption_column = caption_column.split(",") if "," in caption_column else [caption_column]
        if len(caption_column) != 2:
            raise ValueError(
                f"--caption_column' value '{args.caption_column}' needs to be in the format 'caption_column,mask_column'"
            )
        if caption_column[0] not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.caption_column}' needs to be one of: {', '.join(column_names)}"
            )
        if caption_column[1] not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.caption_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    # TODO: preprocess the image and the mask
    ...
    # text_encoders = [align_encoder_one, text_encoder_two]
    # tokenizers = [tokenizer_one, tokenizer_two]
    # tokenizers = [align_tokenizer_one, align_tokenizer_two]
    ...
    compute_mae_encodings_fn = functools.partial(compute_mae_encodings, mae=mae, mae_device=mae_device)
    with accelerator.main_process_first():
        ...
        # )
        train_dataset_with_mae = train_dataset.map(
        ...
        precomputed_dataset = concatenate_datasets(
            ...
            [train_dataset_with_mae, train_dataset_with_vae.remove_columns(["image", "masked_image", "mask"])], axis=1
        )
        precomputed_dataset = precomputed_dataset.with_transform(preprocess_train)

    # del compute_vae_encodings_fn, compute_embeddings_fn, text_encoder_one, text_encoder_two
    # del compute_vae_encodings_fn, compute_mae_encodings_fn, text_encoder_one, text_encoder_two
    del compute_vae_encodings_fn, compute_mae_encodings_fn
    ...
    def collate_fn(examples):
        ...
        mae_embeds = torch.stack([torch.tensor(example["mae_embeds"]) for example in examples])

        return {
            "model_input": model_input,
            # "prompt_embeds": prompt_embeds,
            "mae_embeds": mae_embeds,
    ...
    # )
    unet, align_tokenizer_one, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, align_tokenizer_one, optimizer, train_dataloader, lr_scheduler
    )
    ...
    # TODO: add the mae and vae embeddings in training loop
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        for step, batch in enumerate(train_dataloader):
            # with accelerator.accumulate(unet):
            with accelerator.accumulate(align_tokenizer_one):
                ...
                prompt_embeds_list = []
                bs_embed, seq_len, _ = align_embeds_1.shape
                prompt_embeds_1 = align_embeds_1.view(bs_embed, seq_len, -1)
                prompt_embeds_list.append(prompt_embeds_1)
                ...
                pooled_prompt_embeds: (1, 77)
                add_time_ids: torch.Size([1, 6])
                '''
                if args.unet_onnx:
                    model_preds =[]
                    for unet_model in unet_list:
                        # unet_input = {
                        ...
                        noisy_model_input = noisy_model_input.cpu().numpy()
                        timesteps = timesteps.cpu().numpy()
                        prompt_embeds = prompt_embeds.cpu().numpy()
                        add_time_ids = add_time_ids.cpu().numpy()
                        noisy_model_input = noisy_model_input.astype(np.float32)
                        add_time_ids = add_time_ids.astype(np.float32)
                        pooled_prompt_embeds = pooled_prompt_embeds.astype(np.float32)
                        prompt_embeds = prompt_embeds.astype(np.float32)
                        # NOTE: check the type of all value float or double
